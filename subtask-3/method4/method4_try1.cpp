...
void* queue_threads(void *arg) {
    ...
    ans[arg_struct->frame_number] = queue_d;
    cout << arg_struct->frame_number << " " << queue_d << "\n";
...
//------------------------------------------------------------------------------------------
    ...
    // struct data_for_threads args[(int) n + 100];
    ...
    while(1) {
        ...
        // pthread_create(&ptids[c], NULL, queue_threads, (void *) &args[d]);
        pthread_create(&ptids[c], NULL, queue_threads, (void *) &temp);
